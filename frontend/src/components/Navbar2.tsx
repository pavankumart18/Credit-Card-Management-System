...
const Navbar2: React.FC<Navbar2Props> = ({ notifications = [], onNavigate, initialTheme = "dark", user }) => {
  ...
  useEffect(() => {
    ...
    else if (path === '/cards') setActive('Cards');
    else if (path === '/transactions') setActive('Transactions');
    else if (path === '/redeems') setActive('Redeems');
    else if (path === '/statements') setActive('Statements');
            ...
            {showProfile && (
              <motion.div initial="initial" animate="animate" exit="exit" variants={menuMotion} className="absolute right-0 mt-3 bg-gray-900/95 backdrop-blur-xl border border-gray-700 rounded-2xl shadow-xl p-3 w-48">
                ...
                <ul className="mt-2 text-gray-300 text-sm">
                  ...
                  <li>
                    <button onClick={handleLogout} className="w-full text-left p-2 hover:bg-red-800/30 rounded-md flex items-center gap-2 text-red-400"><LogOut size={14} /> Logout</button>
                  </li>
                </ul>
              </motion.div>
            )}

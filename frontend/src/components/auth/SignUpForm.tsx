...
const PersonalInfoStep = ({ formData, errors, handleChange, handlePasswordChange, handlePasswordVisibility }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
        ...
        <Input id="age" label="Age (Optional)" type="number" value={formData.age} onChange={handleChange} />
        ...
        <Input id="pan" label="PAN Number (Optional)" value={formData.pan} onChange={(e) => handleChange({ target: { id: 'pan', value: e.target.value.toUpperCase() } })} error={errors.pan} maxLength={10} />
        <Input id="aadhaar" label="Aadhaar Number (Optional)" value={formData.aadhaar} onChange={handleChange} error={errors.aadhaar} maxLength={14} />
    </div>
);
...
const DocumentsStep = ({ formData, handleFileChange, errors }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 min-h-[150px]">
        {formData.employmentType === 'Salaried' && <FileUpload id="salarySlips" label="Latest Salary Slips" file={formData.salarySlips} onChange={handleFileChange('salarySlips')} error={errors.salarySlips} />}
        <FileUpload id="photoId" label="PAN/Aadhaar Card Copy" file={formData.photoId} onChange={handleFileChange('photoId')} error={errors.photoId} />
    </div>
);
...
export const SignUpForm = ({ setView }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        phone: '', pan: '', aadhaar: '', age: '', username: '',
        employmentType: '', company: '', yearsOfExperience: '',
        annualIncome: '', existingLoanAmount: '',
    ...
    const validateStep = (): boolean => {
        ...
        if (currentStep === 0) {
            ...
            if (formData.pan && !VALIDATION_RULES.pan.test(formData.pan.toUpperCase())) newErrors.pan = 'Please enter a valid PAN number.';
            if (formData.aadhaar && !VALIDATION_RULES.aadhaar.test(formData.aadhaar)) newErrors.aadhaar = 'Please enter a valid Aadhaar number.';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const signupData = {
                ...
                aadhaar: formData.aadhaar.replace(/\s/g, '') || undefined,
                ...
                estimated_existing_loan_amount: formData.existingLoanAmount ? parseFloat(formData.existingLoanAmount) : undefined,
            };
                ...
                <form onSubmit={handleSubmit} className="mt-8">
                    ...
                    {currentStep < 5 && (
                        <div className="flex justify-between mt-10">
                            <button type="button" onClick={handleBack} disabled={currentStep === 0 || loading} className="px-6 py-3 rounded-lg font-semibold bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed transition">Back</button>
                            <button
                                type={currentStep === 4 ? "submit" : "button"}
                                onClick={currentStep < 4 ? handleNext : undefined}
                                disabled={loading}
                                className="px-6 py-3 rounded-lg font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 hover:scale-105 transition-transform shadow-lg shadow-blue-600/30 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                            >
                                {loading ? 'Submitting...' : (currentStep === 4 ? 'Submit Application' : 'Next Step')}
                            </button>
                        </div>
                    )}
                </form>

  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!month || !year || expiryMonth < 1 || expiryMonth > 12) {
        show("Invalid expiry format. Use MM/YY", 'error');
        return;
      }
      ...
      await addCard({
        ...
        card_name: card.title || `${card.cardBrand} Card`,
        credit_limit: parseFloat(card.creditLimit),
        due_date: 5,
      });

      show("Card added successfully!", 'success');
      navigate("/cards");
    } catch (err: any) {
      show(err.message || "Failed to add card", 'error');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white flex flex-col items-center p-6">
      ...
      <div className="grid grid-cols-1 gap-4 w-96">
        <input
          name="title"
          placeholder="Card Name (e.g., HDFC Millennia)"
        ...
        />
        <button
          className="bg-yellow-400 text-black font-bold rounded-2xl py-3 shadow-lg hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
          onClick={handleSave}
          disabled={loading}
        >
        ...
        <button
          className="bg-gray-700 text-white font-bold rounded-2xl py-3 shadow-lg hover:scale-105 transition-all duration-300"
          onClick={() => navigate(-1)}
        >
          Cancel
        </button>

...
// Removed sampleCards - now using real API data
...
export default function MyCardsPage(): JSX.Element {
    ...
    // Real API hooks
    const { cards: apiCards, loading, error, blockCard, unblockCard, updateCardPin, deleteCard: deleteCardApi } = useCards();
    ...
    useEffect(() => {
        if (apiCards && apiCards.length > 0) {
            const transformedCards = apiCards.map((card: any) => ({
                ...
                subtitle: card.subtitle || card.card_type || '',
                ...
                cvv: '***',
                brand: card.card_brand || card.brand || '',
    ...
    function handleAddCard(payload: Omit<Card, "id" | "masked" | "blocked">) {
        // Navigate to add card page (already has API integration)
        navigate('/add-card');
        setShowAdd(false);
    }
    ...
    async function toggleBlockCard(id: string) {
        try {
            const card = cards.find(c => c.id === id);
            if (card?.blocked) {
                await unblockCard(id);
                show('Card unblocked successfully', 'success');
            } else {
                await blockCard(id);
                show('Card blocked successfully', 'success');
            }
            // Cards will auto-refresh via useCards hook
        } catch (err: any) {
            show(err.message || 'Failed to update card status', 'error');
        }
    }
    ...
    async function handleDeleteCard(id: string) {
        ...
        try {
            ...
            show('Card deleted successfully', 'success');
            // Cards will auto-refresh via useCards hook
        } catch (err: any) {
            show(err.message || 'Failed to delete card', 'error');
        }
    }
    ...
    async function handleSetPin(cardId: string, pin: string) {
        try {
            await updateCardPin(cardId, pin);
            show('PIN updated successfully', 'success');
            setShowPinFor(null);
        } catch (err: any) {
    ...
    function handleSetPinOld(cardId: string, pin: string) {
        setShowPinFor(null);
